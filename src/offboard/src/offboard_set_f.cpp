...
int main(int argc, char *argv[])
{
   ...
   // mavros_msgs:: cmd;
    // offboard_pkg::info uav_msg;

  //the setpoint publishing rate MUST be faster than 2Hz
    ros::Rate rate(200.0);
    double a_x,a_y,a_z,a_xy,max_a_xy;

    bool take_off = false;
    bool reach_height = true;
    ...
    pose.pose.position.z = 10;  // 起飞到1米高度
    // 等待连接
 ...
 while (ros::ok())
    {
        ...
        position_target.type_mask = mavros_msgs::PositionTarget::IGNORE_PX | mavros_msgs::PositionTarget::IGNORE_PY |
                                    ...
                                    mavros_msgs::PositionTarget::IGNORE_AFZ | mavros_msgs::PositionTarget::FORCE;            a_x=geometry_control.a_fb(0);
            a_y=geometry_control.a_fb(1);
            a_z=geometry_control.a_fb(2);
                // 设置期望的加速度值（单位为m/s^2）
          a_x = std::max(-2.0, std::min(a_x, 2.0));
          a_y= std::max(-2.0, std::min(a_y, 2.0));
          a_z= std::max(-2.0, std::min(a_z, 2.0));
        position_target.acceleration_or_force.x =  a_x; // x轴方向加速度
        position_target.acceleration_or_force.y = a_y; // y轴方向加速度
        position_target.acceleration_or_force.z = a_z; // z轴方向加速度
    ...
    pose_pub_.publish(pose);
            ...
            // if( reach_height){
               // ROS_INFO("publishing pose");
                 //pose_pub_.publish(pose);
               // if (abs(geometry_control.mavPos_[2] - 1) < 0.1){ reach_height = false;}
               // }
     //else{
        //ROS_INFO("publishing acc");
